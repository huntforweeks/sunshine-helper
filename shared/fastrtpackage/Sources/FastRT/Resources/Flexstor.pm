...
sub read_Flexstor_file {
	...
	$file =~ s/\"//g;
	my $new_file = $filepath.$file;
	my $stat = open(FP,$new_file);
	if (!$stat) {
	    $new_file = $filepath."../$file";
	    $stat= open(FP,$new_file);
	    if (!$stat) {
		$new_file = $filepath."../../$file";
		open(FP,$new_file) || warn("Can not open include file $file");
	    }
	}
	my @inc_content = <FP>;
	close(FP);
...
sub table
{
	...
	if ( grep /$table_identifier/, $line ) {
	    ...
	    $i = $i_end; #+ 1; (Arve Kyllings original code) i.e.,  Advance $i beyond this table
	}
    }
    return @tables;
}
